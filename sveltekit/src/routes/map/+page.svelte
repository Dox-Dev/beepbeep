<script lang="ts">
  ...
  function check_if_layer_is_in_map(layer: VectorLayer) {
    const isLayerExistsInMap = mountedMap
      .getLayers()
      .getArray()
      .includes(layer);
  ...
  let selectedSource: Location | null = null;
  let selectedDestination: Location | null = null;
  ...
  function put_pin_on_map(
    ...
    if (pin_type == "source") {
      ...
      if (check_if_layer_is_in_map(sourceLayer))
        mountedMap.removeLayer(sourceLayer);

      // make map pin and put in vector source
      let map_pin_feature = new Feature({
        geometry: new Point(fromLonLat([Number(long), Number(lat)])),
        style: mapPinIconStyle,
        name: location_name,
      });
      const sourceSource = new VectorSource({
        features: [map_pin_feature],
      });
      ...
      if (check_if_layer_is_in_map(destinationLayer))
        mountedMap.removeLayer(destinationLayer);
      ...
      const destinationSource = new VectorSource({
        features: [map_pin_feature],
      });
  ...
  async function confirm_locations() {
    if (selectedSource && selectedDestination) {
      ...
      console.log("Confirmed Locations:", selectedSource, selectedDestination);

      // Call to getRoute action to fetch the OSRM route
      const routeResponse = await fetch("/getRoute", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          selectedSource: `${selectedSource.lon},${selectedSource.lat}`,
          selectedDestination: `${selectedDestination.lon},${selectedDestination.lat}`,
        }),
      });
      ...
      alert("Select both starting location and destination please.");
    }
  }

  function displayRoute(route: OSRMRoute) {
  ...
  function selectLocation(location: Location, isSource: boolean) {
    if (isSource) {
...
<div class="overflow-hidden h-screen relative flex z-0">
  ...
  <div class="grid grid-cols-2">
    <!-- Top left drawer-->
    <Button size="xs" color="light" class="absolute left-4 top-4"
      ><IconShape /></Button
    >
    <Dropdown>
    ...
    <ButtonGroup class="*:!ring-primary-700 absolute right-4 top-4">
      <Button size="xs">
        <div class="flex space-x-4 rtl:space-x-reverse">
          <Avatar
            size="xs"
            src="https://s3-alpha.figma.com/hub/file/4338730091/25fd46ec-d7b2-4052-830e-674d22fe7f8c-cover.png"
          />
        </div>
      </Button>
  ...
  {#if is_searching_source || is_searching_destination}
    <Card class="absolute m-auto bottom-4 left-0 right-0 mt-4">
      <div class="grid grid-cols-[1fr_5fr_1fr] mb-8 mx-2">
        ...
        <div></div>
        <button class="text-end"> Done </button>
      </div>

      <div class="grid grid-cols-[1fr_3fr] mb-8">
        <div>
          <Button pill={true} class="!w-fit !h-fit !p-2"
            ><IconCircleFilled class="h-4 w-4" /></Button
          >
        </div>
        <div>
          <form method="POST" action="?/searchLocation" use:enhance>
            ...
            {:else if is_searching_destination}
              <Input
                ...
                id="large-input"
                size="sm"
                placeholder="Where to? Choose destination"
              />
            {/if}
          </form>
        </div>
        ...
        {#each form?.geocode_json as location}
          <Button
            ...
            on:click={() => {
              ...
              put_pin_on_map(
                is_searching_source ? "source" : "destination",
                location.display_name,
                location.lat,
                location.lon
              );
            }}>{location.display_name}</Button
          >
        {/each}
      {/if}
    </Card>
    ...
    <Card class="absolute m-auto bottom-4 left-0 right-0 mt-4">
      <div class="grid grid-cols-[1fr_3fr_1fr] mb-8">
        ...
        <div class="grid grid-rows-3 justify-center">
          <Button pill={true} class="!w-fit !h-fit !p-2"
            ><IconMapPinFilled class="h-4 w-4" /></Button
          >
        ...
        <div class="grid grid-rows-3">
          {#if source_location_first}
            <Button
              ...
              color="alternative"
              size="xs"
              on:click={() => show_search_card("destination")}
            >
              <div class="text-xs">
                <span>Where to?</span>
            ...
            <Button
              ...
              color="alternative"
              size="xs"
              on:click={() => show_search_card("source")}
            >
              <div class="text-xs">
        ...
        <div class="grid grid-rows-3">
          <div></div>
          <button on:click={switch_location_inputs}>
            <IconArrowsUpDown
              on:click={switch_location_inputs}
              class="h-4 w-4"
            />
          </button>
          <div></div>
      ...
      <div>Suggested Travel Destinations</div>
      ...
      <div class="mt-4 text-center">
        <Button on:click={confirm_locations} color="primary">
          Confirm Locations
        </Button>
      </div>
    </Card>
